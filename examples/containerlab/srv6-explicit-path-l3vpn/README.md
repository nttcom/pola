...
```bash
$ sudo docker exec -it clab-srv6-explicit-path-l3vpn-pola-pce bash
...
```bash
$ sudo ip netns exec clab-srv6-explicit-path-l3vpn-pe01 tcpdump -nni eth1
...
```bash
ssh $clab_host "sudo -S ip netns exec clab-srv6-explicit-path-l3vpn-pe01 tcpdump -U -nni eth1 -w -"  | wireshark -k -i -
```
